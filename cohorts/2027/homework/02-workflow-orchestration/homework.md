...
> [!NOTE]
> This is a draft carried over from the 2026 cohort. The questions, datasets and
> deadlines will be updated before the module starts.
...
The homework release contains seven monthly files for each taxi service:

| Service | Files to ingest |
| --- | --- |
| Yellow taxi | `yellow_tripdata_2021-01.csv.gz` through `yellow_tripdata_2021-07.csv.gz` |
| Green taxi | `green_tripdata_2021-01.csv.gz` through `green_tripdata_2021-07.csv.gz` |

As a hint, Kestra makes that process really easy:
1. You can leverage the backfill functionality in the [scheduled flow](flows/09_gcp_taxi_scheduled.yaml) to backfill the data for the year 2021. Just make sure to select the time period for which data exists i.e. from `2021-01-01` to `2021-07-31`. Also, make sure to do the same for both `yellow` and `green` taxi data (select the right service in the `taxi` input).
