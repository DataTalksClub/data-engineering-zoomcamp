...
- Preparation: original JPG is `640×360`, SHA-256
  `f7049dec5f91ca5325a52a693d094eef83d867e05dc52e013cc38892fc1c42c9`.
  The true native crop was reproduced directly from that JPG with
  `convert 06-deploying-a-machine-learning-model-01-exported-to-gcs.jpg
  -crop 530x330+110+25 +repage`, i.e. `(x=110, y=25, width=530, height=330)`;
  its SHA-256 is
  `67d98cb508edee7e66e90c4585b86c7382492c96b569666cc43fdf6559ab37b8`.
  Independent ImageMagick comparison reports `AE=0`; the old crisp/upscaled
  derivative was not used as the source of truth.
- Method: deterministic vector-backed PNG sibling
  `06-deploying-a-machine-learning-model-01-exported-to-gcs-crisp.png`.
  Exact Cloud Storage UI evidence was rendered from the verified source
  labels and layout rather than regenerated by imagegen. The non-instructional
  cursor and the two black recording marks over `Created` and `Public access`
  were removed; the legitimate filter caret, deleted-data toggle, and row
  kebab were retained.
- Invariants checked: `taxi-rides-ny`, `Bucket details`, bucket name
  `taxi_ml_model`, `tip_model/`, `europe-west3 (Frankfurt)`, `Standard`,
  `Not public`, `None`, `OBJECTS`, `CONFIGURATION`, `PERMISSIONS`,
  `PROTECTION`, `LIFECYCLE`, all six action labels, exact filter phrase
  `Filter by name prefix only`, `Filter objects and folders`, `Show deleted
  data`, and the object-table labels/state remain visible; browser tab chrome,
  faces, camera tiles, cursors, and unrelated overlays are absent.
- C2PA/evidence: deterministic final PNG contains no C2PA/JUMBF/AI metadata
  markers; source, native-crop, final, and 608px hashes provide provenance.
- Validation: final native output is `1590×990`, SHA-256
  `dc34eb721fc25cf8cd2db90e0b67340817f6f7a0e9d9325c7b7c2e901e9b81c7`;
  proportional `608×379` render SHA-256 is
  `1b8bdfbc2896e80c48071db8b01b3af15262849a867345e83c17bb1b2eeffe0a`.
  Native and 608px renders were visually inspected for readability and
  clipping; Markdown reference resolves and `git diff --check` passes.
