...
def format_to_parquet(src_file, dest_file):
	if not src_file.endswith('.csv'):
		...
		return
	table = pd.read_csv(src_file,low_memory=False,keep_default_na=False)
	for col in table:
		if col=='lpep_pickup_datetime':
...
with DAG(
    ...
    while execution_date <= end_date:
        ...
        execution_date_t=execution_date.strftime("%Y-%m")
        url_template = f"{URL_PREFIX}/green_tripdata_{execution_date_t}.csv"
        local_csv_path_template= f"{AIRFLOW_HOME}/green_tripdata_{execution_date_t}.csv"
        local_parquet_path_template = f"{AIRFLOW_HOME}/green_tripdata_{execution_date_t}.parquet"
        gcs_path_template = f"raw/green_tripdata/{execution_date_t[:4]}/green_tripdata_{execution_date_t}.parquet"
