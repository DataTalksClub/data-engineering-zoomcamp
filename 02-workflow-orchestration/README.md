...
### Extension: Ingesting FHV data

The course flows only process the `yellow` and `green` taxi types, but you can extend them to the FHV dataset as well. Add these tasks to [`08_gcp_taxi.yaml`](flows/08_gcp_taxi.yaml) based on the previous ones.

First, guard the FHV tasks with an `If` and create the destination table:
...
Then create the external table and the staging table with the hashed `unique_row_id`:
...
And merge into the destination:
...
And extend the `taxi` input with the new value:
